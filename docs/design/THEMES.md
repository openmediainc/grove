...
| `speech(bubble)` | a laid-out bubble (box, wrapped lines, tail/leader, "+N", whisper) — placement is decided by `@grove/ui` speech-layout, never the theme. Crowds (#64): leaders ≤ 2.5 tiles, never crossing; lines a crowd can't place fold into ONE cluster bubble (`bubble.cluster`: `"lines"` = newest line + "+N more", `"count"` = far-zoom number), drawn by `kit.drawSpeechBubble` as a stack; tap opens the room drawer on those lines | dark pill | cyan terminal | comic white | mono teal |
