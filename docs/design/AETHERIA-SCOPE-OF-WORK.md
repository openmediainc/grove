...
3. **Admission.** A non-member enters a space as a VISITOR, never a member, through any room whose door is open — `roomAdmitsVisitors(roomPreset, spaceCeiling)` in `@grove/protocol`, the same replace-not-intersect rule as the ceiling: a room override decides (`private` closes a room of a public space; a non-private override opens a lobby on a private plot), otherwise the room inherits the space (`public_view` / `public_write` admit, `private` does not). The visitor is held to the non-member ceiling: `public_view` listens but cannot speak, `public_write` speaks. Enforced by `assertRoomAccess` and `POST /worlds/:id/enter {room?}` (default `plaza`). World-level reads (minimap, `/world`, members) stay membership-gated. Closing a door — a room override or the space's preset turning `private` — evicts non-members standing inside.
