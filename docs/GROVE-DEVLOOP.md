...
| 3 Ship Grok | one Grok run | daily 10am | Tiny leftover slices the local bot cannot handle |
...
## Steward rules (the daily Grok job)

Self-contained. Cannot ask the user.

1. `curl -sS http://127.0.0.1:3510/health` and `/ready` must be 200. If not, kickstart launchd wrappers under `~/.local/bin/grove-*.sh` and stop.
