...
export default function StudioAgent() {
  ...
  if (!agent) return <main className="p-12 text-white/50">Loading studio…</main>;
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-4xl text-lantern-300">{agent.display_name}</h1>
          <p className="text-sm text-white/50">{agent.slug}</p>
        </div>
      </div>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2">
        {TOGGLES.map((t) => (
          <button
            ...
            className={`rounded-2xl border p-5 text-left ${agent.policy[t.key] ? "border-lantern-400 bg-lantern-400/10" : "border-white/10 bg-dusk-800/70"}`}
          >
        ...
        <div className="mt-3 flex gap-2">
          {["hang_out", "await_orders", "work", "perform", "scribe"].map((m) => (
            <button
              ...
              className={`rounded-full px-4 py-1 text-sm ${agent.autonomy_mode === m ? "bg-lantern-400 text-dusk-950" : "border border-white/15"}`}
            >
              {m.replace("_", " ")}
            </button>
          ))}
      ...
      <section className="mt-8">
        ...
        />
        <button onClick={sendOrder} className="mt-2 rounded-full bg-white/10 px-4 py-1">
          Save standing order
        </button>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 flex gap-2">
          ...
          />
          <button onClick={oneShot} className="rounded-full bg-lantern-400 px-4 text-dusk-950">
            Send
          </button>
        </div>
      </section>
      ...
      <section className="mt-8">
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {keys.map((k) => (
            <li key={k.id} className="flex items-center justify-between rounded-lg bg-dusk-800/80 p-3">
              <span>
                {k.prefix}… {k.revoked_at ? "(revoked)" : ""} {k.last_used_at ? `last used ${k.last_used_at}` : ""}
              </span>
              {!k.revoked_at ? (
                <button onClick={() => revoke(k.id)} className="text-red-300">
                  Revoke
                </button>
              ) : null}
            </li>
