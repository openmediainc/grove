...
export class GroveApp {
  ...
  constructor(pg: Pool, redis: Redis, config: GroveConfig) {
    ...
    this.speech = new SpeechService(this.store, this.flags, this.quota, this.presence);
    this.observe = new ObserveService(this.store, this.presence, this.speech, this.identity);
    this.world = new WorldService(this.store, this.presence, this.identity, this.flags);
    this.moderation = new ModerationService(this.store, this.quota, this.identity);
  }
}
