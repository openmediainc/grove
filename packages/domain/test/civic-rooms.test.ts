...
import { utcDay } from "../src/services/notices.js";
import {
  assertTestDatabase,
  clearActorLimiters,
...
describe.skipIf(!hasDb)("civic rooms have a mechanic", () => {
  ...
  it("gives a UTC day's pin to the first notice of that day, and to nothing else", async () => {
    const day = utcDay();
    // The pin is a global daily slot, so whether it was already held before this
    // test ran changes what MY first post gets — but not the invariant, which is
    // the thing worth asserting either way.
    const { rows: before } = await pg.query<{ n: string }>(
      `SELECT count(*)::text AS n FROM notices WHERE pinned_on = $1::date`,
      [day],
    );
    const dayWasFree = Number(before[0]!.n) === 0;
    ...
    if (dayWasFree) expect(first.tookPin).toBe(true);
    ...
    const { rows: after } = await pg.query<{ n: string }>(
      `SELECT count(*)::text AS n FROM notices WHERE pinned_on = $1::date`,
      [day],
    );
  ...
  it("keeps yesterday's pin off today's board", async () => {
    ...
    await pg.query(
      `INSERT INTO notices (id, author_id, author_kind, title, body, pinned, pinned_on, created_at)
       VALUES ($1, $2, 'human', $3, 'old news', TRUE, DATE '2000-01-02', TIMESTAMPTZ '2000-01-02T09:00:00Z')`,
      [`notice_old_${tag()}`, author.id, `Old ${tag()}`],
    );
    ...
    expect(board.pin?.pinnedOn).not.toBe("2000-01-02");
  });
